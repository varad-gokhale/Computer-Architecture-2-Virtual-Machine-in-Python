...
startexecptr = 0;
def regval ( rstr ):                                 # help with reg or indirect addressing; returns register number
                                                     # if direct addressing, else sets the Direct bit and ret value
                                                     # eg - regval (3): ret 0011; regval(*3): ret 1011
    if rstr.isdigit():
       return ( int( rstr ) )
    elif rstr[0] == '*':
...
mem = [0] * memloadsize                              # this is the memory load executable, a list of memories, size=1024
...
for line in infile.readlines():           # read our asm code
   ...
   if firsttoken == 'go':                               # start execution here
      ...
      continue
   if firsttoken[0] == '.':
      symboltable[firsttoken] = curaddr    # save the (relative?)address of the current instruction to symboltable list
   curaddr = curaddr + 1
print symboltable
# END Build Symbol Table

#This section takes op-code through the in.asm file, parses, builds a hex code
infile.close()
infile = open("in.asm", 'r')
for line in infile.readlines():           # read our asm code
   tokens = string.split( string.lower( line ))        # tokens on each line
   ...
   instype = opcodes[ tokens[0] ][0]                       # dictionary_name[key][element number in list]
